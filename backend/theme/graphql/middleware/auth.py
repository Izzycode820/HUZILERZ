"""
Theme Authentication Middleware - Hybrid Public/Private Access (v3.0)

Handles both public (theme store browsing) and authenticated (theme management) operations
...
class ThemeAuthMiddleware:
    """
    Conditional authentication for theme GraphQL

    Public operations (no auth required):
    - themes: Browse theme store
    - themeDetails: View theme details

    User-level operations (require JWT but NOT workspace header):
    - addTheme: Clone theme to workspace (workspace_id passed in args)
    - User selects which workspace in the mutation, so no header needed

    Workspace-scoped operations (require JWT + workspace header):
    - myThemes: User's theme library
    - themeCustomization: Get customization for editor
    - publishTheme, updateTheme, etc.
    ...
    PUBLIC_QUERIES = {'themes', 'themeDetails', '__schema', '__type', 'IntrospectionQuery'}

    # User-level operations: require auth but NOT workspace header
    # The workspace is selected in the mutation arguments
    USER_LEVEL_OPERATIONS = {
        'addTheme',  # Clone theme - user selects target workspace in args
    }
    ...
    def resolve(self, next, root, info, **kwargs):
        ...
        # Check if we already determined this is a user-level operation
        ...
        # USER-LEVEL OPERATIONS: Auth required but NO workspace header needed
        # Workspace is passed in mutation arguments instead
